...
class BeeHive:
    ...
    def select_all(self):
        ...
        """
        q = influx_query(f"select * from {self.measurement} where \"sensor-id\"='{self.sensor_id}'")
    ...
    def select_series(self, hours):
        ...
        """
        q = influx_query(f"SELECT * FROM {self.measurement} where \"sensor-id\"='{self.sensor_id}' AND time > now() - {hours}h")
        return list(q.get_points())
    ...
    def mesurement_cnt(self):
        ...
        """
        q = influx_query(f"SELECT COUNT(dht_status) FROM {self.measurement} where \"sensor-id\"='{self.sensor_id}'")
